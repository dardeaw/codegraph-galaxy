...
> **專為多專案與複雜架構打造的高效能 3D 代碼星系視覺化與架構探索套件 —— 內建 AI 對話助理，先查 code 再回答。**  
...
- **全量符號清單，不受 3D 模式限制**：樹狀清單永遠顯示全部符號（函式、方法、import、變數），架構模式在 3D 藏起來也不影響。
- **母子巢狀收折**：方法按限定名收進所屬類別底下，可收可展，顏色與 3D 圖例一致。
- **智慧定位**：點擊 3D 沒顯示的符號，鏡頭自動找最近的可見母節點，不會飛去空座標；檢視器與樹選取留在原節點。
...
### 6. 專案庫存與排除規則維護 (Repositories Management)
...
### 7. AI 代碼對話 (Local LLM Agent)
- **直接問 codebase**：工具列 **AI Chat** 按鈕開啟 Copilot 式面板；本地 LLM 用工具查圖（搜尋 / 呼叫鄰居 / 讀碼 / 影響半徑），不瞎猜。
- **查碼過程**：每個回答附逐步 trace，可點的 kind 色節點 chips；**在圖上顯示**直接打光。
- **母節點走訪**：隱藏節點自動找最近的可見母節點，圖層與 LOD 永不亂動。
- **Explorer 範圍**：搜尋範圍跟著勾選走，不用手選專案，模糊比對、跨輪記憶。
...
npm start
```
`npm install` 會連 pinned 的 CodeGraph indexer 一起拉下來（不用另外裝 CLI），`npm start` 開啟桌面版。AI 對話需要本機模型（Ollama）或配好的服務商 —— 見第 7 節。
...
| **空地點兩下** | 整張圖收回置中 |
| **WASD / 方向鍵** | 飛行（`Shift` 加速） |
| **`Q` / `E`** | 下降 / 上升 |
| **`Enter` / `Shift+Enter`**（AI 對話內） | 送出 / 換行 |
| **`Ctrl + 0`** | 重置相機至預設最佳視角 |
